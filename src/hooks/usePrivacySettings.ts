    ...
    const loadPrivacySettings = async () => {
      try {
        ...
        if (user) {
          ...
          const publicProfile = privacyData && Array.isArray(privacyData) && privacyData.length > 0 
            ? Boolean(privacyData[0]?.public_profile) 
            : true;
            
          const dataSharing = privacyData && Array.isArray(privacyData) && privacyData.length > 0 
            ? Boolean(privacyData[0]?.data_sharing) 
            : false;
            
          const profileVisibility = profileData && Array.isArray(profileData) && profileData.length > 0
